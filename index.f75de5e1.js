...
function slide(row) {
    ...
    for(let i = 0; i < row.length - 1; i++)if (newRow[i] === newRow[i + 1] && isFinite(newRow[i])) {
        newRow[i] *= 2;
        newRow[i + 1] = 0;
        score += newRow[i];
        checkIfWin(newRow[i]);
...
function slideLeft() {
    const initialField = field.map((row)=>[
            ...row
        ]);
...
function slideUp() {
    ...
    for(let col = 0; col < columns; col++){
        ...
        row = slide(row);
        for(let i = 0; i < columns; i++)field[i][col] = row[i];
    }
    if (!isArraysEquals(initialField, field)) setNewCell();
    updateTheGame();
}
...
function isArraysEquals(arrayA, arrayB) {
    for(let i = 0; i < arrayA.length; i++)for(let j = 0; j < arrayA[i].length; j++){
